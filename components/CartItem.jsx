    ...
    <div className='flex py-5 gap-3 md:gap-5 border-b'>
      ...
      <div className='w-full flex flex-col'>
        <div className="flex flex-col md:flex-row justify-between">
            <div className="text-lg md:text-2xl font-semibold text-black/[0.798]">{p.name}</div>
                ...
                <div className="flex items-center gap-1">
                    ...
                    <select className='hover:text-black' onChange={(e) => updateCartItem(e,"selectedSize")}>
                        {p.size.data.map((item,i) =>{
                          return (
                            <option value={item.size} key={i} disabled={!item.enabled ? true : false} selected={data.selectedSize === item.size}>{item.size}</option>
                          )
                        })  
                        }
                    </select>
